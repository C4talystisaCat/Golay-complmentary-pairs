function result = listallpzcpPAR(L, q, zcz)
    % 获取并行工作池对象
    pool = gcp();
    
    % all_sequence 函数生成长度为 L，q 元的所有可能序列
    seq = all_sequence(L, q);
    % extractRowPairs 函数提取所有可能的序列对
    pairs = extractRowPairs(seq);

    % 用于存储计数值 Z
    Z = zeros(1, size(pairs,1)); 
    ...
    parfor i = 1:size(pairs, 1)
        ...
        if count_C1 > 0 && count_C2 > 0
            ...
            if isempty(zcz_value) || Z(i) == zcz_value
                result{i} = {Z(i), f, g}; % 将满足条件的序列对添加到结果中
            end
        end
    end
